...
router.get('/', async (req, res) => {
  try {
    ...
    logger.info(`[/files] Files retrieved successfully for user ${req.user.id}`);
...
router.post('/', async (req, res) => {
  ...
  logger.debug('[/files] File upload request:', req.body);
  let cleanup = true;
  ...
  try {
    ...
    if (metadata.endpoint === 'assistants' && !metadata.assistant_id) {
      throw new Error('Missing assistant_id for assistant uploads.');
    }

    // Ensure assistant_id is included
    metadata.assistant_id = metadata.assistant_id || req.body.assistant_id;
    ...
    try {
      ...
      logger.error('[/files] Error deleting file1:', error);
    }
